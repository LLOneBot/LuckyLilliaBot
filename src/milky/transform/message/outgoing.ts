  ...
  for (const segment of segments) {
    try {
      if (segment.type === 'text') {
        ...
        const resid = await ctx.qqProtocol.uploadForward(peerUid, isGroup, raw.multiMsgItems)
  ...
  async visit(content: OutgoingForwardedMessage) {
    ...
    for (const segment of content.segments) {
      ...
      if (type === 'text') {
        ...
        const resid = await this.ctx.qqProtocol.uploadForward(this.peerUid, this.isGroup, innerRaw.multiMsgItems)

...
export class NTQQWebApi extends Service {
  ...
  /** 拉群公告列表 GET https://web.qun.qq.com/cgi-bin/announce/list_announce */
  ...
  /** 删除群公告 POST https://web.qun.qq.com/cgi-bin/announce/del_feed */
  async deleteGroupBulletin(groupCode: string, feedsId: string) {
    const cookieObject = await this.ctx.ntUserApi.getCookies('qun.qq.com')
    const bkn = this.genBkn(cookieObject.skey)
    const url = 'https://web.qun.qq.com/cgi-bin/announce/del_feed'
    const res = await fetch(url, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
  ...
  /** 上传群公告图片 POST https://web.qun.qq.com/cgi-bin/upload_pic */
  async uploadGroupBulletinPic(groupCode: string, filePath: string): Promise<{ errCode: number, errMsg: string, picInfo?: { id: string, width: number, height: number } }> {
    ...
    const formData = new FormData()
    formData.append('bkn', bkn)
    formData.append('source', 'qun_announce')
    formData.append('qid', groupCode)
    formData.append('pic_up', new Blob([new Uint8Array(buf)], { type: ft?.mime || 'image/jpeg' }), `pic.${ft?.ext || 'jpg'}`)
    const res = await fetch('https://web.qun.qq.com/cgi-bin/upload_pic', {
      method: 'POST',
      headers: { 'Cookie': this.cookieToString(cookieObject) },
      body: formData,
    })
    ...
    if (data.ec !== 0) {
      return { errCode: data.ec, errMsg: data.em || '', picInfo: undefined }
    }
    return {
      errCode: 0,
      errMsg: '',
      picInfo: { id: data.id, width: data.w, height: data.h },
    }
  }

  /** 拉群精华消息 GET https://qun.qq.com/cgi-bin/group_digest/digest_list */
  async queryCachedEssenceMsg(groupCode: string, pageStart = 0, pageLimit = 20) {
    const cookieObject = await this.ctx.ntUserApi.getCookies('qun.qq.com')
    const bkn = this.genBkn(cookieObject.skey)

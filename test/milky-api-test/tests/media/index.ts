/**
 * milky 包共享 onebot11-api-test 的媒体资源（避免重复维护两份 fixture）。
 * tsconfig.json 用 paths 别名 `@/...` 指向 milky 包 src；这里直接 file:// 读 OB11 包的 tests/media/*.
 */
...
// milky-api-test/tests/media/ → onebot11-api-test/tests/media/
...
function generateFreshVideo(): string {
  if (cachedFreshVideoPath && existsSync(cachedFreshVideoPath)) return cachedFreshVideoPath
  ...
  // 跟 OB11 测试保持同样的 fontfile 配置（gyan portable ffmpeg drawtext 需要显式 fontfile）
...
export const MediaPaths = {
  ...
  testMp4Path: p('test.mp4'),
  get freshVideoPath() { return generateFreshVideo() },
  get freshVideoUri() { return 'file:///' + generateFreshVideo().replace(/\\/g, '/') },
  resolve: p,
  toUri: fileUri,
}
